...
- **限流实现从 KV 换成 D1**：`TECH_DESIGN.md:359` 写的是"沿用 KV 固定窗口"。改成 D1 `rate_limit` 表是为了根除 F-E（读改写非原子）与 F-B（键长上限）。代价：热路径每请求多 1 次 D1 写，实测登录端到端从修复前的 **≈23ms（账号不存在）/ ≈43ms（密码错）** 变为 **≈68ms**（交错采样中位数），注册与 OIDC 端点同理多一次写。对"≤50 人的朋友局"规模可接受；若日后要压回来，可考虑 Durable Object 计数器或"KV + 定期校准"，但两者都比现在复杂，且都不解决"键长进键名"的输入面。**用户已于 2026-09-18 认可，偏差定案。**
